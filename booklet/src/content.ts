 ...
 * file:line rail. Nothing is invented. Two deliberate scoping calls, grounded
 * in the code rather than a single ambiguous "speedup":
 *
 *   · NATIVE speedups (3.50× matmul-256, etc.) are `openmp+native` vs
 *     `single-thread`. The hand-written SIMD is compiled into ALL THREE
 *     configs (NEON on the M2 that produced the run), so these bars measure
 *     threading + native codegen ON TOP OF SIMD — not SIMD vs scalar.
 *     (benchmarkData.ts:17-24; LandingPage.tsx:385-387)
...
export const TOC = {
  chapterTaglines: {
    ...
    PROOF: "scoped speedups · 97.01% · 41 + 29 tests",
...
// loop retires 2× that per iteration. Source · src/NeuralNet.cpp (dispatch
// :22-32; kernels 49-74 / 95-124 / 146-165 / 195-215).
// ---------------------------------------------------------------------------
...
export const ISAS = [
  ...
  },
  {
    ...
    accentKey: "03_INSIDE" as SectionKey,
    note: "The kernel that produced the committed M2 run — 128-bit ARM vectors, two f64 lanes, dual accumulators.",
  },
  {
    id: "wasm128",
    n: "4",
...
export const ISA_META = {
  ...
  source: "source · src/NeuralNet.cpp:22-32 (guards) · :232-238 (dispatch)",
} as const;
...
export const HOW = {
  ...
  dispatch: {
    ...
    source: "source · src/NeuralNet.cpp:22-32 (guards) · :232-238 (kernel dispatch)",
  },
  ...
  neon: {
    ...
    body: [
      "On Apple silicon and other ARM chips the kernel is `dot_neon_rowvec`: 128-bit `float64x2_t` vectors, two doubles per lane, two accumulators, FMA where the chip supports it. It is the exact kernel that produced this card's committed benchmark run, on an Apple M2.",
      "NEON is only two doubles wide — narrower than AVX — but the same dual-accumulator shape keeps the pipeline full, and on the M2 it feeds a memory system fast enough that the vectorized loop is no longer the bottleneck.",
    ],
    stat: { value: "float64x2", label: "128-bit ARM vector · 2 lanes" },
    stat2: { value: "M2", label: "the committed run's silicon" },
  ...
  threads: {
    ...
    body:
      "On big operations, spreading work across cores helps: `matmul 256×256` runs 3.50× faster with `openmp+native` than single-thread. On small ones it hurts — thread startup costs more than the work saved, so `transpose 128` gets 4.3× worse. The fix is a tuned size threshold in the code, not a blanket `parallel for`.",
    bands: [
      { range: "≥ 4096 elems", verb: "PARALLELIZE", tone: "green", detail: "matmul 256×256 → 3.50× faster (omp+native)." },
      { range: "small ops", verb: "STAY SERIAL", tone: "amber", detail: "transpose 128 → 4.3× worse if threaded." },
      { range: "tiny net", verb: "SKIP THREADS", tone: "steel", detail: "classify: baseline beats openmp+native." },
    ],
    loopNote:
      "The pragma is the receipt: `if (rows_ * rhs.cols_ >= 4096)` — parallelize only past the size where it pays.",
    source: "source · benchmarkData.ts:26-90 · src/Matrix.cpp (schedule(static) if …≥4096)",
  },
} as const;
...
export const INSIDE = {
  ...
  anatomy: {
    ...
    source: "source · benchmarkData.ts:9-15 · Workbench.tsx:238-239 · src/NeuralNet.cpp:245",
  },
} as const;
...
export const PROOF = {
  divider: { subtitle: "scoped speedups · 97.01% · 41 + 29 tests" },
  ...
  speedups: {
    ...
    native: {
      ...
      workload: "matmul 256×256 · Apple M2 · clang 17",
      detail: "4.835 ms → 1.380 ms · 6.9 → 24.3 GFLOP/s",
      caveat:
    ...
    crossover:
      "And the honest counter-example: on small ops OpenMP loses (transpose 128 → 4.3× worse), and on the tiny classify net the plain baseline beats openmp+native outright.",
    source:
      "source · benchmarkData.ts:36 (3.50×) · :73-90 (crossover) · useMnistDemoController.ts:163 (live p50)",
  },
  ...
  accuracy: {
    ...
    source: "source · benchmarkData.ts:11 · LandingPage.tsx:566-567 · TestingSetList.txt (10000 lines)",
  },
  ...
  table: {
    ...
    lede:
      "The committed run, in full — including where parallelism backfires. A speedup you only quote when it wins is not a measurement.",
    wins: [
      { op: "matmul 256×256", base: "4.835 ms", opt: "1.380 ms", factor: "3.50×", win: true },
      { op: "transpose 1024", base: "978 µs", opt: "502 µs", factor: "1.95×", win: true },
      { op: "axpy 1024", base: "231 µs", opt: "115 µs", factor: "2.01×", win: true },
    ],
    losses: [
      { op: "transpose 128", base: "5.4 µs", opt: "23.7 µs", factor: "4.3× worse", win: false },
      { op: "axpy 256", base: "13.9 µs", opt: "26.3 µs", factor: "1.9× worse", win: false },
    ],
    classify:
      "classify throughput: baseline 81,628 img/s beats openmp+native 69,994 — the net is too small to thread. (bench topology 784→30→10, ~24K params — smaller than the shipped model.)",
    method:
      "Google Benchmark · ≥0.5 s CPU per case · mean ns/op · 3 Release configs · run 20251226-154121 · Apple M2, clang 17.",
    reproduce: "python3 tools/run_benchmarks.py --openmp --native",
    source: "source · benchmarkData.ts:26-93 · BENCHMARKS.md · docs/benchmarks/runs/",
  },

  tests: {
    eyebrow: "§04 · THE GUARANTEE",
    headline: "41 C++ tests and 29 in the browser.",
    body:
      "Correctness is enforced, not asserted. The C++ core ships 41 Catch2 test cases — matrix algebra, the network, property-based checks with RapidCheck, and the server API. The web app adds 29 executed Playwright end-to-end tests that drive the real demo in a headless browser.",
    stats: [
      { value: "41", label: "C++ tests · Catch2 + RapidCheck", note: "tests/*.cpp" },
      { value: "29", label: "e2e · Playwright · executed", note: "8 tests × 4 projects − 3 skips" },
      { value: "4", label: "ISAs · one shared kernel shape", note: "NeuralNet.cpp" },
    ],
    breakdown: [
      { k: "test_matrix", v: "20 cases · algebra + SIMD parity" },
      { k: "test_neural_net", v: "13 cases · forward pass + train" },
      { k: "test_matrix_properties", v: "5 cases · RapidCheck properties" },
      { k: "test_server_api", v: "3 cases · the inference endpoint" },
    ],
    honest:
      "The 29 is reconciled: 8 e2e tests across 4 Playwright projects is 32, minus the 3 reduced-motion runs skipped outside their project — 29 actually executed.",
    handoffQuote:
      "A kernel that changes the speed and not the answer is the only kind worth shipping.",
    source: "source · tests/*.cpp (41 TEST_CASE) · web/playwright.config.ts · web/tests/e2e/demo.spec.ts",
  },
} as const;
...
export const BUILD = {
  ...
  pipeline: {
    ...
    source: "source · CMakeLists.txt:278-281 (-msimd128) · tools/build_wasm.sh · CHANGELOG.md",
  },
  ...
  stack: {
    ...
    rows: [
      ...
      { area: "TESTS", tech: "Catch2 · RapidCheck · Playwright", note: "41 C++ · 29 e2e · Google Benchmark" },
    ],
    source: "source · CMakeLists.txt:1,47 · docs/WASM.md:30 · web/package.json:31,56",
  },
