 ...
 *   SlopeChart      — GFLOP/s 7.0 → 24.6 on the matmul win.               [slope]
...
export const ScatterQuadrant: React.FC<{ pts: ScatterPt[]; accent?: string; source?: string }> = ({
  ...
  // y floor is 0.08 so transpose 128 (0.127× on the M1 Pro reference) plots
  // inside the frame rather than under it; ceiling clears transpose 1024 (3.92×).
  const ylo = 0.08, yhi = 5.0; // factor (log)
  ...
  const yticks = [0.1, 0.25, 0.5, 1, 2, 4];
  return (
    <ChartCard label="speedup vs baseline cost" accent={accent} source={source}>
      ...
      <Caption>Above the line, threading pays; below, it costs. Cost alone does not decide it — axpy 1024 is expensive and still lands below the line.</Caption>
    </ChartCard>
  );
};
...
export const PerIterUnits: React.FC<{ accent?: string }> = ({ accent = COLORS.AMBER_DEEP }) => {
  ...
  const gap = 3;
  return (
    <ChartCard label="doubles / iteration" accent={accent} source="NeuralNet.cpp:49-123">
