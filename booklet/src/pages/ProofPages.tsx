  ...
  return (
    <BodyPage {...p} sectionLabel="PROOF" sectionColor={GREEN_INK} eyebrow={d.eyebrow} headline={d.headline}>
      ...
      <div style={{ marginTop: 16, display: "grid", gridTemplateColumns: "1.25fr 0.75fr", columnGap: 20, alignItems: "stretch" }}>
        <SlopeChart leftLabel="1-thread" rightLabel="omp+native" leftValue={7.0} rightValue={24.6} unit="GFLOP/s" delta="+3.54×" accent={GREEN_INK} source="benchmarkData.ts:36" />
        <div style={{ border: `0.5pt solid ${COLORS.HAIRLINE}`, borderLeft: `3px solid ${GREEN_INK}`, borderRadius: 6, background: COLORS.PAPER_ELEVATED, padding: "14px 16px", display: "flex", flexDirection: "column", justifyContent: "center", gap: 8 }}>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 8, flexWrap: "wrap" }}>
            <span style={{ fontFamily: FONTS.MONO, fontSize: 16, fontWeight: 700, color: COLORS.STEEL_DEEP, fontVariantNumeric: "tabular-nums" }}>4.819ms</span>
            <span style={{ fontFamily: FONTS.MONO, fontSize: 12, color: COLORS.INK_SUBTLE }}>→</span>
            <span style={{ fontFamily: FONTS.MONO, fontSize: 20, fontWeight: 700, color: GREEN_INK, fontVariantNumeric: "tabular-nums" }}>1.363ms</span>
          </div>
          <div style={{ fontFamily: FONTS.MONO, fontSize: 9, fontWeight: 600, color: COLORS.INK_MUTED }}>−72% · matmul 256×256</div>
          <div style={{ fontFamily: FONTS.SERIF, fontStyle: "italic", fontSize: 10.5, lineHeight: 1.3, color: COLORS.INK_MUTED, borderTop: `0.5pt solid ${COLORS.HAIRLINE}`, paddingTop: 7 }}>
            Threading + native codegen, measured on the M1 Pro — on top of the SIMD already in every config.
          </div>
        </div>
      </div>
  ...
  return (
    <BodyPage {...p} sectionLabel="PROOF" sectionColor={GREEN_INK} eyebrow={d.eyebrow} headline={d.headline}>
      ...
      <div style={{ marginTop: 2 }}>
        ...
        <div style={{ fontFamily: FONTS.MONO, fontSize: 7.5, fontWeight: 700, letterSpacing: "0.1em", color: COLORS.AMBER_DEEP, margin: "10px 0 2px" }}>LOSSES — where threading costs</div>
...
// M1 Pro reference medians. axpy 1024 crossed over between machines: its dot
// now sits to the RIGHT of the baseline, in amber, where it belongs.
const TABLE_DUMB: DumbRow[] = [
  { op: "matmul 256×256", base: 4819, opt: 1363, factor: "3.54×", win: true },
  { op: "transpose 1024", base: 876, opt: 224, factor: "3.92×", win: true },
  { op: "axpy 1024", base: 273, opt: 296, factor: "1.08× worse", win: false },
  { op: "axpy 256", base: 14.9, opt: 49.4, factor: "3.3× worse", win: false },
  { op: "transpose 128", base: 5.7, opt: 45.0, factor: "7.9× worse", win: false },
];

// ── p22 · proof-tests — 37 + 57 ────────────────────────────────────────────
export const ProofTestsPage: React.FC<PageProps> = (p) => {
  ...
  return (
    <BodyPage {...p} sectionLabel="PROOF" sectionColor={GREEN_INK} eyebrow={d.eyebrow} headline={d.headline}>
      ...
      {/* the 37, by suite — a stacked composition of the C++ cases */}
      <div style={{ marginTop: 16, borderTop: `1pt solid ${GREEN}`, paddingTop: 12 }}>
        <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 8 }}>
          <span style={{ fontFamily: FONTS.MONO, fontSize: 8.5, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: GREEN_INK }}>37 C++ cases, by suite</span>
        ...
        <div style={{ display: "flex", gap: 3, height: 22 }}>
          {[
            { n: "matrix", c: 18, col: COLORS.GREEN_DEEP },
            { n: "neural_net", c: 11, col: COLORS.SKY_DEEP },
        ...
        <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginTop: 7 }}>
          {[
            { n: "test_matrix · 18", col: COLORS.GREEN_DEEP },
            { n: "test_neural_net · 11", col: COLORS.SKY_DEEP },
          ...
          <span style={{ fontFamily: FONTS.MONO, fontSize: 8, color: COLORS.INK_SUBTLE }}>· + 57 Playwright e2e</span>
        </div>
      </div>
