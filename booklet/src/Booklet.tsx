...
import { WhyScalarPage, WhyFloorPage, WhyHotloopPage } from "./pages/WhyPages";
 ...
 * layout, so whatever this returns scales every overflow delta it reports.
 * Basing the fit on the one-up leaf keeps that factor near 1 (0.911 at
 * clipcheck's 840×1080 viewport, where the 96px chrome inset binds); basing it
 * on the *spread* width instead would put the gate at ~0.5 and halve every
 * threshold it checks. clipcheck additionally neutralises this transform before
 * measuring, so it reads true page pixels — but the cap stays, because the
 * gate should not be the only thing standing between a change and a
 * silently desensitised measurement.
 */
...
> = {
  "why-scalar": WhyScalarPage,
  "why-floor": WhyFloorPage,
  "why-hotloop": WhyHotloopPage,
  "how-dispatch": HowDispatchPage,
  "how-avx": HowAvxPage,
  "how-neon": HowNeonPage,
  "how-wasm": HowWasmPage,
  "how-threads": HowThreadsPage,
  "inside-wasm": InsideWasmPage,
  "inside-bench": InsideBenchPage,
  "inside-anatomy": InsideAnatomyPage,
  "proof-speedups": ProofSpeedupsPage,
  "proof-accuracy": ProofAccuracyPage,
